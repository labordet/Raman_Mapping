...
from tkinter import Tk, filedialog, Label, Button, Entry, ttk, messagebox
import os
...
class SpectraAnalysisApp:
    def __init__(self, root):
        ...
        # ========== (1) File + Directory Selection Frame ==========
        self.file_frame = ttk.Frame(root)
        self.file_frame.pack(fill="x", padx=10, pady=10)

        Label(self.file_frame, text="Select .txt File:").grid(row=0, column=0, sticky="e")
        self.file_label = ttk.Label(self.file_frame, text="No file selected", width=50)
        self.file_label.grid(row=0, column=1, sticky="w")
        Button(self.file_frame, text="Browse", command=self.select_file).grid(row=0, column=2, padx=5)

        Label(self.file_frame, text="Select Output Directory:").grid(row=1, column=0, sticky="e")
        self.dir_label = ttk.Label(self.file_frame, text="No directory selected", width=50)
        self.dir_label.grid(row=1, column=1, sticky="w")
        Button(self.file_frame, text="Browse", command=self.select_directory).grid(row=1, column=2, padx=5)

        # ========== (2) Hidden Frame for Analysis Inputs & Plot ==========
        self.analysis_frame = ttk.Frame(root)
        # We'll pack() this frame only AFTER a file is chosen.

        # Region Start/End, Points/Lines
        Label(self.analysis_frame, text="Region Start:").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        self.region_start = Entry(self.analysis_frame, width=10)
        self.region_start.grid(row=0, column=1, sticky="w", padx=5, pady=5)
        self.region_start.insert(0, "240")  # Default

        Label(self.analysis_frame, text="Region End:").grid(row=1, column=0, sticky="e", padx=5, pady=5)
        self.region_end = Entry(self.analysis_frame, width=10)
        self.region_end.grid(row=1, column=1, sticky="w", padx=5, pady=5)
        self.region_end.insert(0, "254")  # Default

        Label(self.analysis_frame, text="Points per Line:").grid(row=2, column=0, sticky="e", padx=5, pady=5)
        self.points_per_line = Entry(self.analysis_frame, width=10)
        self.points_per_line.grid(row=2, column=1, sticky="w", padx=5, pady=5)
        self.points_per_line.insert(0, "110")  # Default

        Label(self.analysis_frame, text="Lines per Image:").grid(row=3, column=0, sticky="e", padx=5, pady=5)
        self.lines_per_image = Entry(self.analysis_frame, width=10)
        self.lines_per_image.grid(row=3, column=1, sticky="w", padx=5, pady=5)
        self.lines_per_image.insert(0, "90")  # Default

        # Analyze button
        Button(self.analysis_frame, text="Analyze", command=self.analyze).grid(row=4, column=0, columnspan=2, pady=10)
        ...
        # Let the lower rows/columns expand if the user resizes
    ...
    def select_directory(self):
        ...
        self.output_dir = filedialog.askdirectory(title="Select the directory to save results")
        if self.output_dir:
            self.dir_label.config(text=self.output_dir)

    def load_and_plot(self):
        """Load the data, compute the average, plot it, and show the rest of the UI."""
